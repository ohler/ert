...
The behavior of this function is changed in two points if `noninteractive' 
is non-nil.  (`noninteractive' is set to non-nil if the current emacs process 
is started with --batch option.)

1. If the optional argument SELECTOR is `nil', the all tests are selected as
like `t' is given.

2. With an exit status the emacs process is terminated with `kill-emacs'.
The exit status is 0 if all test cases are resulted as expected, and is 1 if 
any test case is resulted as unexpected.

Above two points are considered following use-case to run test:

 $ emacs --batch -L . --load your-tests.el -f ert-run-tests-batch
 $ echo $?

 With behavior 1. ert-run-tests-batch can be passed as the function (-f)
to run all test cases in batch mode.

 With behavior 2. the test result can be known as exit status($? in sh/bash) 
from the shell command line. "
  (let* ((selector (if (and noninteractive (null selector)) 
		       t 
		     selector))
	 (stats (ert-run-tests
		selector
		  ...
		  (ecase event-type
		    ...
		    (test-ended
			 ...
			 (message format-string
				  ...
				  (ert-test-name test))))))))))
    (if noninteractive
	(kill-emacs (if (zerop (+ (ert-stats-passed-unexpected stats)
				  (ert-stats-failed-unexpected stats)
				  (ert-stats-error-unexpected stats)))
			0
		      1))
